...
export default function HeroSection({ 
  ...
  const effectiveSlides = slides && slides.length > 0
    ? slides
    : (backgroundImage || backgroundVideo || title)
      ? [{
          subtitle,
          title,
          description,
          mediaType: backgroundVideo ? 'video' : (backgroundImage ? 'image' : 'none'),
          mediaUrl: backgroundVideo || backgroundImage,
          ctaText: showCTA ? ctaText : '',
          ctaLink,
          secondaryCTA,
          secondaryCTALink,
        }]
